...
{
    public partial class TextAdsProduct : System.Web.UI.UserControl
    {
        ...
        public void BindTextAdsProduct(int collegeId)
        {

            try
            {
                var objBannerData = _objCommon.GetTextAdsProduct(collegeId);
                if (objBannerData != null && objBannerData.Tables.Count > 0)
                {
                    diVtextAds.Visible = true;
                    rptTextProduct.Visible = true;
                    rptTextProduct.DataSource = objBannerData.Tables[0];
                    rptTextProduct.DataBind();
                }
                else
                {
                    diVtextAds.Visible = false;
                    rptTextProduct.Visible = false;
                ...
                (Exception ex)
            {
                ...
                const string addInfo =
                    "Error in Executing  BindTextAdsProduct in UserControl/TextAdsProduct.ascx :: -> ";
                var objPub = new ClsExceptionPublisher();
                objPub.Publish(err, addInfo);
            }
        }
        protected void rptTextProduct_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            try
            {
                if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    if (rptProductDiscount != null)
                    {
                        ...
                        var dt =
                            objCommon.GetTextAdsProductDiscount(
                        ...
                        if (dt != null && dt.Tables.Count > 0)
                        {
                            if (dt.Tables[0].Rows.Count > 0)
                            {
                                ...
                                for (rowCount = 0; rowCount <= dt.Tables[0].Rows.Count - 1; rowCount++)
                                {
                                    var productAmount =
                                        Common.ConvertRupee(
                                            Convert.ToString(dt.Tables[0].Rows[rowCount]["AjAmount"]));
