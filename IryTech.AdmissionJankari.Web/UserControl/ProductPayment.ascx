<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ProductPayment.ascx.cs" Inherits="IryTech.AdmissionJankari.Web.UserControl.ProductPayment" %>
<asp:UpdatePanel ID="updateState" runat="server">
    <ContentTemplate>
        <div class="boxPlane mainBG">
            ...
            <center> <asp:Button ID="btnFinish" CssClass="button" runat="server" OnClientClick="SetHiddenFieldValue()"   Text="Finish"  
        ...
        <asp:HiddenField runat="server" ID="hdnAmount" ClientIDMode="Static" />
        <input type="hidden" id="hdnPageClickCount" value="0" />
    </ContentTemplate>
</asp:UpdatePanel>
...
<script type="text/javascript" defer="defer">
    ...
    function SetHiddenFieldValue() {
        $("#hdnPageClickCount").val("1");
    }
    $(document).ready(function () {
        CheckStatus();
        $('#<%= rbtnPaymentType.ClientID %>').change(function () {

              $("#<%=lblCash3.ClientID %>").html($("#hdnAmount").val());
              $("#<%=lblCash2.ClientID %>").html($("#hdnAmount").val());
              $("#<%=lblCash1.ClientID %>").html($("#hdnAmount").val());
            if ($("#hdnPageClickCount").val() == "0")
                CheckStatus();
        });
    });
