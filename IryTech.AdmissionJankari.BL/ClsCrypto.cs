...
<Start Date				: -	 2011-Oct
<End Date				: -	
<Description			: -   Cryptographic Services for BRICS based on Rijndael(rain-dahl) Algorithim 	
<								
<								Random symetric key / Password Phrase algorithim 
<								to avoid BruteForce & Dictionary attcks
<Review Date			: - 
<Reviewed By			: - 	
...
namespace IryTech.AdmissionJankari.BL
{
	...
	{
		string HASHALGO_MD2  = "MD2";
		string HASHALGO_MD4  = "MD4";
		string HASHALGO_MD5  = "MD5";
		string HASHALGO_RIPEMD = "RIPEMD";
		string HASHALGO_SHA1  = "SHA1";
		string HASHALGO_Snefru  = "Snefru";
		string HASHALGO_Tiger  = "Tiger";

		
		/// <summary>
		/// This function will generate a 16 Byte InitVector based on two numbers. 
		/// This pass can be futher user to create instance of ClsCrypto for encryption.
		/// While decrypting use the same numbers used at the time of encrypting data to 
		/// generate InitVector otherwise crypto service will not be able to dycrypt it properly
		/// </summary>
		/// <param name="Base1">Pass Base 1 for InitVector (CustomerID)</param>
		/// <param name="Base2">Pass Base 2 for InitVector (PolicyID)</param>
		/// <returns>InitVector </returns>
		public static string GetInitVector(int Base1, int Base2)
		{
			...
			for (int i= 1 ; i<= 16 ; i++)
			{
				...
				if(i%2 != 0)
				{
					...
					{	
							//Use First digit 
						case 1:
							InitVector[i-1] = tmp.ToString().ToCharArray()[0];		
							break;
							//Use loop iterator
						case 7: 
							InitVector[i-1] = '@';
							break;
							// Use ASCII equivalent of Iterator	
						case 3:
							InitVector[i-1] = (char)(i);
							break;
							//Use Ascii Equiv of Last Digit
						case 15:
							int LastDigit = tmp%10;
							InitVector[i-1] = (char) LastDigit;		
							break;
							//Use ASCII equiv of number itself make if less tha 100 if not
						case 5:
							while(true)
							{
								if(tmp < 100)
							...
							InitVector[i-1] = (char) tmp;		
							break;
						//Use ASCII equiv of result itself make if less tha 128 if not
						case 9:
							tmp = tmp * i + Base2;
							while(true)
							{
						...
						case 13:
							...
							InitVector[i-1] = (char) tmp;		
							break;
								//Use ASCII equiv of number itself make if less tha 95 if not
						case 11:
							while(true)
							{
								if(tmp < 100)
				...
				else
				{
					...
					switch(i)
					{
						...
						case 8:
							...
							InitVector[i-1] = (char) LastDigit;		
							break;
							//Use Ascii Equiv of First Digit
						case 10:
							int FirstDigit = 79;
							while(true)
							{
							...
							InitVector[i-1] = (char) FirstDigit;		
							break;
							//Use ASCII equiv of number itself make if less tha 128 if not
						case 12:
							while(true)
							{
								if(tmp < 128)
		...
		/// <summary>
		/// This function will generate a 24 Byte Pass Key based on two numbers. 
		/// This pass can be futher user to create instance of ClsCrypto for encryption.
		/// While decrypting use the same numbers used at the time of encrypting data to 
		/// generate pass key otherwise crypto service will not be able to dycrypt it properly
		/// </summary>
		/// <param name="Base1">Pass Base 1 for PassPhrase (CustomerID)</param>
		/// <param name="Base2">Pass Base 2 for PassPhrase (PolicyID)</param>
		/// <returns>24 byte PasswordPhrase string value </returns>
		public static string GetPasswordPhrase(int Base1, int Base2)
		{
			char [] PassPhrase = new char[24];
			...
			for (int i= 1 ; i<= 24 ; i++)
			{
				...
				if(i%2 != 0)
				{
					...
					{	
							//Use First digit 
						case 1:
							PassPhrase[i-1] = tmp.ToString().ToCharArray()[0];		
							break;
							//Use loop iterator
						case 7: 
							PassPhrase[i-1] = '7';
							...
							// Use ASCII equivalent of Iterator	
						case 3:
							PassPhrase[i-1] = (char)(i);
							break;
							//Use Ascii Equiv of Last Digit
						case 15:
							int LastDigit = tmp%10;
							PassPhrase[i-1] = (char) LastDigit;		
							break;
							//Use Ascii Equiv of First Digit
						case 19:
							int FirstDigit = 79;
							...
							PassPhrase[i-1] = (char) FirstDigit;		
							break;
							//Use ASCII equiv of number itself make if less tha 128 if not
						case 17:
							while(true)
	...
	/// <summary>
	/// Cryptographic Services for BRICS based on Rijndael Algorithim 	
	///	with Salt to avoid BruteForce & Dictionary attcks
	/// </summary>
	public class ClsCrypto
	{
		...
		/// in secret.
		/// </param>
		public ClsCrypto(string  passPhrase) : 
		...
		public string Decrypt(string cipherText)
        {
            ...
            try
            {
                decryptData = Decrypt(Convert.FromBase64String(cipherText));
            ...
            catch (Exception ex)
            {
                ...
                const string addInfo = "Error while executing Decrypt in ClsCrypto.cs  :: -> ";
