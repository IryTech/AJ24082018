...
using IryTech.AdmissionJankari.Components.Web.Extensions;
using IryTech.AdmissionJankari.Components.Web.HttpHandlers;
...
namespace IryTech.AdmissionJankari.Components
{
    ...
    public static class Utils
    {
        ...
        /// </summary>
        private static string _applicationRelativeWebRoot;
        ...
        private static readonly Regex RegexMobile =
            new Regex(
                ConfigurationManager.AppSettings.Get("Apps.MobileDevices"), 
        ...
        /// <summary>
        ///     Boolean for returning whether or not Ramuakaka is currently running on Mono.
        /// </summary>
        ...
        /// <summary>
        ///     Gets the relative URL of the Ramuakak feed. If a Feedburner username
        ...
        /// <summary>
        ///     Gets the relative root of the current Ramuakak instance.
        /// </summary>
        /// <value>A string that ends with a '/'.</value>
        public static string RelativeWebRoot
        {
            ...
            get
            {
                return Applications.CurrentInstance.RelativeWebRoot ?? "~/";
            }
        }
        ...
        public static string ApplicationRelativeWebRoot
        {
            get
            {
                return _applicationRelativeWebRoot ??
                       (_applicationRelativeWebRoot =
                        VirtualPathUtility.ToAbsolute(ApplicationConfig.VirtualPath));
            }
        }

        /// <summary>
        ///     Gets if the current HTTP request is a homepage request, taking the Ramukaka
        ...
        public static bool IsCurrentRequestForHomepage
        {
            get
            {
                ...
                // If this is a virtual folder for a Ramukaka instance, unless "default.aspx" is
                // actually in the URL, default.aspx won't be reported in path, so we check
                // to see if path is the root of RelativeWebRoot (the Ramukaka instance's
                // relative web root).

                if (RelativeWebRoot.Equals(VirtualPathUtility.AppendTrailingSlash(path), StringComparison.OrdinalIgnoreCase))
                    return true;
                else if (path.Equals(string.Format("{0}default.aspx", Utils.ApplicationRelativeWebRoot)))
                    return true;
        ...
        public static ListItem SelectListItemByValue(ListControl control, string value)
        {
            ...
            foreach (var li in control.Items.Cast<ListItem>().Where(li => string.Equals(value, li.Value, StringComparison.OrdinalIgnoreCase)))
            {
                li.Selected = true;
                return li;
            }
        ...
        //public  void AddJavaScriptIncludeADVST(System.Web.UI.UserControl page,string url, bool placeInBottom, bool addDeferAttribute, bool minify)
        //{
        //    if (placeInBottom)
        //    {
        ...
        //        {
        //            Page.ClientScript.RegisterClientScriptBlock(page.getType(), "ThirdPartyAdvst.ascx.css", script);
        //        }
        //    }
        //    else
        //    {
        ...
        //        jqueryInclude.Attributes.Add("src", url);
        ...
        //        page.Controls.AddAt(0, jqueryInclude);
        //        if (context != null) { context.Items[itemsKey] = ++nextInsertPosition; }
        //    }
        //}
            ...
            bool isFrontEnd)
        {
            ...
            var scriptsAddedDuringRequest = HttpContext.Current.Items[itemsKey] as Dictionary<string, bool>;
            ...
            var knownItems = new List<ExternalContentItem>()
            {
                new ExternalContentItem() { ItemName = "jquery-1.7.2.min.js", Priority = 0, Defer = false, Minify=false },
                new ExternalContentItem() { ItemName = "Autocomplete.js", Priority = 1, Defer = false, IsFrontEndOnly = true, AddAtBottom = false, Minify = true },
                new ExternalContentItem() { ItemName = "CommonFrontScript.js", Priority =2, Defer = false, IsFrontEndOnly = true, AddAtBottom = false, Minify = true },
            };
            ...
            fileEntries.Sort((x, y) =>
            {
                ...
                return xPriority == yPriority ? System.String.Compare(xFile, yFile, System.StringComparison.Ordinal) : xPriority.CompareTo(yPriority);
            });
        ...
        /// <summary>
        /// This method adds the resource handler script responsible for loading up Ramukaka's culture-specific resource strings
        /// on the client side. This needs to be called at a point after Ramukaka.js has been added to the page, otherwise this script
        ...
        {
            var resourcePath = ResourceHandler.GetScriptPath(new CultureInfo("en-GB"));
        ...
        public static IEnumerable<Assembly> CodeAssemblies()
        {
            ...
            try
            {
                ...
                {
                    assemblyName = "IryTech.AdmissionJankari.BL";
                    codeAssemblies.Add(Assembly.Load(assemblyName));
                }
            }
        ...
        /// <summary>
        /// Returns the default culture.  This is either the culture specified in the application settings,
        ...
        {
            var settingsCulture = ApplicationSettings.Instance.Culture;
        ...
        private static readonly Regex MobileRegex = new Regex(
            @"^[7-9][0-9]{9}$", RegexOptions.IgnoreCase);
        ...
        {
            // Here we will get the all user to get the mail id from user id
            return userName;
        }
        ...
        public static bool IsMobileValid(string mobileNo)
        {
            ...
            {
                return MobileRegex.IsMatch(mobileNo.Trim());
            }
        ...
        public static void LoadExtensions()
        {
            ...
            foreach (var a in codeAssemblies)
            {
        ...
        public static string SendMailMessage(MailMessage message)
        {
            ...
            try
            {
                ...
                var smtp = new SmtpClient(ApplicationSettings.Instance.SmtpServer);

                // don't send credentials if a server doesn't require it,
                // linux smtp servers don't like that 
                if (!string.IsNullOrEmpty(ApplicationSettings.Instance.SmtpUserName))
                {
                    smtp.Credentials = new NetworkCredential(
                        ApplicationSettings.Instance.SmtpUserName, ApplicationSettings.Instance.SmtpPassword);
                }

                smtp.Host = ApplicationSettings.Instance.Host;
                smtp.Port = ApplicationSettings.Instance.SmtpServerPort;
                smtp.Send(message);
                OnEmailSent(message);
            }
            catch (Exception ex)
        ...
        {
            // Before entering a BG thread, retrieve the current instance application settings.
            Guid ramukakaId = Applications.CurrentInstance.Id;
            ThreadPool.QueueUserWorkItem(delegate
            {
                // because HttpContext is not available within this BG thread
                // needed to determine the current Ramukaka instance,
                // set override value here.
                Applications.InstanceIdOverride = ramukakaId;
        ...
        public static bool SetConditionalGetHeaders(DateTime date)
        {
            ...
            if (System.String.CompareOrdinal(incomingEtag, etag) == 0 || incomingLastModifiedDate == date)
            {
                response.Clear();
        ...
        ///     attributed set to "Ramukaka" it will
        ...
        public static string RemoveIllegealFromCourse(string text)
        {
