...
namespace IryTech.AdmissionJankari.Components
{
    ...
    /// 
    /// RemoteFile makes use of two Ramukaka properties: AllowServerToDownloadRemoteFiles and RemoteFileDownloadTimeout.
    /// 
    ...
    internal sealed class RemoteFile
    {
        ...
        internal RemoteFile(Uri filePath, bool ignoreRemoteDownloadSettings)
        {
            ...
            this._timeoutLength = ApplicationSettings.Instance.RemoteFileDownloadTimeout;
        }
        ...
        public WebResponse GetWebResponse()
        {
            ...
            if (contentLength <= -1)
            {
                ...
                throw new SecurityException(
                    "An attempt to download a remote file has been halted due to unknown content length.");
            }
            else if ((ApplicationSettings.Instance.RemoteMaxFileSize > 0) &&
                     (contentLength > ApplicationSettings.Instance.RemoteMaxFileSize))
            {
                response.Close();
                throw new SecurityException(
        ...
        {
            if (!this.IgnoreRemoteDownloadSettings && !ApplicationSettings.Instance.AllowServerToDownloadRemoteFiles)
            {
        ...
        /// <summary>
        /// Gets whether this RemoteFile instance is ignoring remote download rules set in the current Ramukaka setting instance.
        /// </summary>
        /// <remarks>
        /// 
        ...
        {
            get { return this._ignoreRemoteDownloadSettings; }
        }
        ...
        {
            get { return (this.Uri.AbsoluteUri.StartsWith(Utils.AbsoluteWebRoot.AbsoluteUri)); }
        }
        ...
        /// in the current Ramuakaka setting instance. 
        /// 
        ...
        public int TimeoutLength
        {
            get
            {
                return (this.IgnoreRemoteDownloadSettings
                            ? this._timeoutLength
                            : ApplicationSettings.Instance.RemoteFileDownloadTimeout);
            }
            set
            {
                ...
                {
                    throw new SecurityException(
                        "TimeoutLength can not be adjusted on RemoteFiles that are abiding by remote download rules");
                }
                else
                {
                    if (value < 0)
                    {
                        throw new ArgumentOutOfRangeException("TimeoutLength must be" +
                                                              " a value greater than or equal to 0 milliseconds");
                    }
                    else
                    {
