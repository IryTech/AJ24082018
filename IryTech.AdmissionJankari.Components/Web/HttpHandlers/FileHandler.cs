    ...
    public class FileHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            if (!string.IsNullOrEmpty(context.Request.QueryString["file"]))
            {
                ...
                try
                {
                    var folder = context.Server.MapPath(string.Format("{0}/files", Applications.CurrentInstance.StorageLocation));
                    var info = new FileInfo(folder + Path.DirectorySeparatorChar + fileName);

                    if (info.Directory != null && (info.Exists && info.Directory.FullName.StartsWith(folder, StringComparison.OrdinalIgnoreCase)))
                    {
