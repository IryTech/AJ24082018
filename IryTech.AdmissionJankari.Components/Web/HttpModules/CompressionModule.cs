...
namespace IryTech.AdmissionJankari.Components.Web.HttpModules
{
    ...
    public sealed class CompressionModule : IHttpModule
    {
        ...
        private static void ContextPostReleaseRequestState(object sender, EventArgs e)
        {
            ...
            if (!ApplicationSettings.Instance.EnableHttpCompression) { return; }

            if ((context.CurrentHandler is Page && context.Request["HTTP_X_MICROSOFTAJAX"] == null &&
                context.Request.HttpMethod == "GET") || (context.Request.Url.PathAndQuery.ToLower().Contains(".asmx")))
            {
                CompressResponse(context);

                if (ApplicationSettings.Instance.CompressWebResource)
                {
                    context.Response.Filter = new WebResourceFilter(context.Response.Filter);
                    WillCompressResponse = true;
                }
            }
            else if (!ApplicationSettings.Instance.CompressWebResource && (context.Request.Path.Contains("WebResource.axd") || context.Request.Path.Contains("ScriptResource.axd")))
            {
